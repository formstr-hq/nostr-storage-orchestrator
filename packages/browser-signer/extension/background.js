...
var HashMD = class {
  ...
  digestInto(out) {
    ...
    for (let i2 = 0; i2 < outLen; i2++)
      oview.setUint32(4 * i2, state2[i2], isLE3);
  }
  digest() {
...
// ../../node_modules/.pnpm/nostr-tools@2.23.5_typescript@6.0.3/node_modules/nostr-tools/lib/esm/index.js
...
// ../../node_modules/.pnpm/nostr-tools@2.23.5_typescript@6.0.3/node_modules/nostr-tools/lib/esm/nip49.js
...
// src/core.ts
var state = { activeSecret: null, activeNpub: null };
var idleTimer = null;
...
async function importKey(nsec, passphrase, label) {
  const decoded = nip19_exports.decode(nsec.trim());
  if (decoded.type !== "nsec") throw new Error("Not an nsec");
  const secret = decoded.data;
  const ncryptsec = encrypt3(secret, passphrase);
  const npub = nip19_exports.npubEncode(getPublicKey(secret));
...
async function generateKey(passphrase, label) {
...
async function setOriginAllowed(origin, allowed) {
  const list = await getAllowlist();
  const next = allowed ? [...list.filter((entry) => entry.origin !== origin), { origin, addedAt: Date.now() }] : list.filter((entry) => entry.origin !== origin);
...
async function handle(message, sender) {
  switch (message.type) {
    case "status":
      return await status();
    case "unlock":
      return await unlock(message.npub, message.passphrase);
    case "lock":
      return lock();
    case "importKey":
      return await importKey(message.nsec, message.passphrase, message.label);
    case "generateKey":
      return await generateKey(message.passphrase, message.label);
    case "removeProfile":
      return await removeProfile(message.npub);
    case "allowlist":
      return await getAllowlist();
    case "setOriginAllowed":
      await setOriginAllowed(message.origin, message.allowed);
      return { ok: true };
...
async function requireAllowed(sender) {
