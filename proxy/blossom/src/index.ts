...
app.head("/upload", async (req, res) => {
  try {
    const xHash = firstHeader(
      req.headers["x-sha-256"] as string | string[] | undefined,
    );
    const npub = verifyAuthToken(req.headers.authorization, "upload", {
      hash: xHash,
      requireHashScope: true,
    });

    const contentLengthHeader = firstHeader(
      req.headers["x-content-length"] as string | string[] | undefined,
    );
    ...
    if (Number(user.usedStorage) + size > limits.storageLimit) {
      res.setHeader(
        "X-Reason",
        "Storage limit exceeding while uploading this file.",
      );
...
app.put("/upload", async (req, res) => {
  try {
    ...
    if (!Buffer.isBuffer(data) || data.length === 0) {
      ...
      return res
        .status(400)
        .json({ error: "Request body must contain binary data" });
    }

    const hash = sha256Hex(data);
    const size = data.length;

    const clientHash = firstHeader(
      req.headers["x-sha-256"] as string | string[] | undefined,
    );
    if (clientHash && clientHash.toLowerCase() !== hash) {
      res.setHeader(
        "X-Reason",
        "X-SHA-256 header does not match the uploaded content",
      );
      return res
        .status(409)
        .json({
          error: "X-SHA-256 header does not match the uploaded content",
        });
    }

    const authHeader = req.headers.authorization;
    const npub = verifyAuthToken(authHeader, "upload", {
      hash,
      requireHashScope: true,
    });
    ...
    if (Number(user.usedStorage) + size > limits.storageLimit) {
      ...
      return res.status(403).json({
        error:
          "Storage limit exceeding while uploading this file. Please upgrade your plan to upload this file.",
      });
    }
    if (size > limits.uploadLimit) {
      res.setHeader("X-Reason", "File exceeds upload limit.");
      return res.status(403).json({
        error:
          "File exceeds upload limit. Please upgrade your plan to upload this file.",
      });
    }
    ...
    const contentType =
      firstHeader(
        req.headers["content-type"] as string | string[] | undefined,
...
async function handleGetBlob(
  ...
  try {
    ...
    return res
      .status(500)
      .json(includeBody ? { error: "Download failed" } : {});
  }
}
...
app.delete("/:hashWithExt", async (req, res) => {
  try {
    ...
    const npub = verifyAuthToken(req.headers.authorization, "delete", {
      hash,
      requireHashScope: true,
    });
